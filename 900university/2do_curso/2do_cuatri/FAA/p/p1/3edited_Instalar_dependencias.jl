...
begin
    ...
    # Por qué? Son las versiones que el profesor usó para desarrollar
    # los tests, desinstalé Julia 
    # y volví a instalar la versión que usó, y ahora mismo estoy
    # haciendo lo mismo con estos paquetes
    # porque el assert de la linea 62 falla por decimas en una funcion
    # que usa valores speudoaleatorios que dependen de la implementacion
    # de un paquete que discrepa en versiones.
    paquetes_con_versiones=[
        Pkg.PackageSpec(name="Flux", version="0.16.7"),
        Pkg.PackageSpec(name="SymDoME", version="1.0.4"),
        Pkg.PackageSpec(name="MLJ", version="0.22.0")
    ]
    Pkg.add(paquetes_con_versiones)
    #Pkg.update();
end
