...
function accuracy(outputs::AbstractArray{<:Real,2}, targets::AbstractArray{Bool,2}; threshold::Real=0.5)
    if size(outputs,2)==1
        #return accuracy(vec(outputs),vec(targets)); Antes
        return accuracy(vec(outputs),vec(targets); threshold=threshold); # Después de correccion.
        # Sí, el error era solo que no estaba usando el threshold
        # que se le pasaba como argumento .-., entonces al hacer 
        # el return y no pasar el threshold usaba el por defecto
