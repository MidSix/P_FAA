# Archivo de pruebas para realizar autoevaluación de 
#algunas funciones de los ejercicios

dir_path_firmas = abspath(@__DIR__, "..", "..");
file_path_firmas = joinpath(dir_path_firmas, "2Firmas.jl");
include(file_path_firmas);
...
# estando las funciones bien, sobre todo en la funciones 
# que implican alguna componente aleatoria
...
# Normalizacion de media 0. en este caso, debido a redondeos, 
# la media y desviacion tipica de cada variable no van a dar 
# exactamente 0 y 1 respectivamente. Por eso las comprobaciones
...
# Test que lanza assert si ocurre el error que le ocurrio al profesor
#(accuracy con matriz columna y threshold != 0.5)
@assert(accuracy(reshape(collect(0.1:0.1:1.0), :, 1), reshape([falses(2); trues(8)], :, 1); threshold=0.8) == 0.5)
